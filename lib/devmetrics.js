...
//        port: 3000,
      ...
      url += '&gauge=' + (gauge || 0);
      var intpref = event_tags && Object.prototype.toString.call( event_tags ) === '[object Array]' ? 'd' : '';
      if (event_tags && (Object.prototype.toString.call( event_tags ) === '[object Array]' || Object.prototype.toString.call( event_tags ) === '[object Object]')) {
        var preparr = [];
        for (var i in event_tags) {
          preparr.push(intpref + i + '|' + event_tags[i].replace(/[\W_]+/g, "_"));
        }
        var tags_str = preparr.join();
        url += '&tags=' + tags_str;
      }
      options['path'] += url;
//      console.log('>>>>>', url);
